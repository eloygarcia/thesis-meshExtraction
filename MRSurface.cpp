...
// Read data.
// --------------------------------------------------------
void MRSurface::read_breastmask(std::string inputfilename)
{
...
// --------------------------------------------------------
void MRSurface::read_segmentation(std::string inputfilename)
{
...
// aux. Functions
// -------------------------------------------------------------
void MRSurface::getRegionOfInterest(ImageMriType::Pointer image, const char* side, ImageMriType::Pointer &new_image)
{
...
// -------------------------------------------------------------
void MRSurface::resampleFunction (ImageMriType::Pointer image, float iso_vox_size, ImageMriType::Pointer &outputimage)
{
	ImageMriType::SizeType size = image->GetLargestPossibleRegion().GetSize();
	ImageMriType::SpacingType spacing = image->GetSpacing();
...
// -------------------------------------------------------------
void MRSurface::closingFunction (ImageMriType::Pointer image, BallType ball, ImageMriType::Pointer &outputImage, int r)
{
...
// -------------------------------------------------------------
void MRSurface::visualizationFunction (ImageMriType::Pointer image, vtkSmartPointer<vtkMarchingCubes> & marchingOutput) //,  vtkSmartPointer<vtkActor> &actorOutput)
{	
...
void MRSurface::Gaussian_solver(std::vector<double> points_interface, float* &solution, int degree)
{	
	if(degree==1)
	{
...
void MRSurface::controlFunction(std::vector<double> &points_interface, std::vector<int> &id_interface,
...
void MRSurface::upAndDownCut(ImageMriType::Pointer image, ImageMriType::Pointer mask, ImageMriType::Pointer &outputimage)
{
...
*/	
	...
	if(degree==1) { std::cout << "Degreeeee = 1" << std::endl; this->solution = new float[3];}
	if(degree==2){ std::cout << "Degreeeee = 2" << std::endl; this->solution = new float[6];}
...
// Meshing
// --------------------------------------------------------
void MRimage::getmodel(vtkSmartPointer<vtkPolyData> &surf_mesh )
{
	vtkSmartPointer<vtkPolyData> surf_mesh = vtkSmartPointer<vtkPolyData>::New();
	getBreastSurfaceModel( this->m_side, surf_mesh);

		new_mesh->set_surfacemesh( surf_mesh );
		new_mesh->improve_quality( surf_mesh );

	new_segmentation->SetSide( this->m_side );
	new_segmentation->SetDegree( this->m_degree );
	new_segmentation->SetFinalSpacing( 1.5 );
	new_segmentation->SetSolution( this->solution );
