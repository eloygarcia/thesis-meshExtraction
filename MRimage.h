...
public:
	...
	// getting poist from mesh
	std::vector<double> getPoints_model();
	std::vector<int> getElements_model();
	std::vector<int> getTissueElements_model();
	std::vector<int> getBoundaryConditions_model();
	vtkSmartPointer<vtkUnstructuredGrid>  get_tetrahedral_mesh(){ return new_mesh->get_tetrahedralMesh();};

	ImageSegmentation3floatType::Pointer getImage_NewSegmentation();
...
private:
	...
	void closeBrestMask(  ImageMriType::Pointer mask, ImageMriType::Pointer &closingMask );
};
